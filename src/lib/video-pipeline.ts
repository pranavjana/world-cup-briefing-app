...
import { connect, type Video } from "videodb";
import {
  buildDemoBriefing,
...
function extractClockMinute(text?: string): number | null {
  if (!text) return null;
...
function isVideoResult(url: string) {
...
export async function searchWorldCupVideos(query: string): Promise<SearchResponse> {
  if (process.env.TINYFISH_API_KEY) {
    try {
      ...
      const results = response.results
        .filter((result) => isVideoResult(result.url))
...
export async function researchMatchInfo(query: string): Promise<ResearchResult> {
  if (!process.env.TINYFISH_API_KEY) {
    return {
      query,
      results: [],
...
export async function runVideoDbBriefing(
  ...
  // Pool every qualifying shot across all labels and queries, then pick the
  // best non-overlapping moments — so "all red cards" returns each card,
  // not just the single top match.
  ...
  // Researched events (e.g. "Red card — Player X") are strong semantic
  // queries in their own right: player names appear in scene descriptions.
  ...
  if (knownEvents?.length) {
    // Researched ground truth: assign the best shot to each known event,
    // scoring by semantic match plus on-screen-clock proximity, and keeping
    // video order consistent with match-minute order.
    ...
    for (const event of ordered.slice(0, MAX_TOTAL_MOMENTS)) {
      ...
      let bestScore = 0.3; // minimum confidence to claim a match
  ...
  if (!chosen.length) {
    // No research available (or nothing matched it): fall back to the
    // generic best-non-overlapping picker.
  ...
  // Scene descriptions let us extend each clip back to the start of the
  // action (the foul before a card, the build-up before a goal).
  ...
  return {
    runId: `vdb-wc-${Date.now().toString(36)}`,
    status: "completed",
    mode: "live",
    topic,
    selectedVideo: {
...
export async function buildLiveOrFallbackBriefing(
  ...
  try {
    return await runVideoDbBriefing(topic, video, onProgress, knownEvents);
  } catch (error) {
    return {
